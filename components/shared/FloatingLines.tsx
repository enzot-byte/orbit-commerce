      ...
      ({
        ...
        if (linesGradient && linesGradient.length > 0) {
          ...
          stops.forEach((hex, i) => {
            ...
            gradientArray[i].set(color.x, color.y, color.z);
          });
        }
        ...
        const renderLoop = () => {
          ...
          if (interactive) {
            const dm = mouseDamping;
            currentMouseRef.current.x += (targetMouseRef.current.x - currentMouseRef.current.x) * dm;
            currentMouseRef.current.y += (targetMouseRef.current.y - currentMouseRef.current.y) * dm;
            uniforms.iMouse.value.set(currentMouseRef.current.x, currentMouseRef.current.y);
            currentInfluenceRef.current += (targetInfluenceRef.current - currentInfluenceRef.current) * dm;
            uniforms.bendInfluence.value = currentInfluenceRef.current;
          }

          if (parallax) {
            const dm = mouseDamping;
            currentParallaxRef.current.x += (targetParallaxRef.current.x - currentParallaxRef.current.x) * dm;
            currentParallaxRef.current.y += (targetParallaxRef.current.y - currentParallaxRef.current.y) * dm;
